...
// A centralized logging function for consistency across extension scripts.
// Set `_debugMode` to `false` in production to suppress all these logs.
const _debugMode = true; // Set to `false` to disable logging
...
chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })
    .then(() => _log("Side panel behavior set successfully (openPanelOnActionClick: true)."))
    .catch((error) => _log("ERROR: Failed to set side panel behavior:", error));
...
chrome.runtime.onInstalled.addListener((details) => {
    _log(`Extension installed or updated. Reason: ${details.reason}.`);
    ...
    chrome.storage.local.remove(['selectedElements', 'context'])
        .then(() => _log("Cleared 'selectedElements' and 'context' from storage on install/update."))
        .catch(error => _log("ERROR: Failed to clear storage on install/update:", error));
});
...
chrome.runtime.onStartup.addListener(() => {
    _log("Service worker starting up. Initializing storage defaults.");
    ...
    chrome.storage.local.set({ selectedElements: [], context: "" })
        .then(() => _log("Storage defaults for selectedElements and context ensured."))
        .catch(error => _log("ERROR: Failed to ensure storage defaults:", error));
});
...
const messageListener = async (message, sender, sendResponse) => {
    _log("Received message in background script:", message);
    ...
    if (message.action === ACTIONS.PROXY_LLM_REQUEST) {
        _log("Handling PROXY_LLM_REQUEST. (Future LLM API call logic would go here).");
        ...
        try {
            ...
            _log("ERROR: Failed to proxy LLM request:", error);
    ...
    _log("INFO: Message action not directly handled by background script (likely direct content/sidepanel communication).");
...
_log("chrome.runtime.onMessage listener registered.");
...
async function cleanupExtension() {
    _log("Starting background script cleanup (onSuspend/unload).");
    ...
    try {
        ...
        _log("chrome.runtime.onMessage listener removed.");
    } catch (error) {
        _log("WARN: Error removing message listener (might have already been removed or not active):", error);
    }
    ...
    try {
        ...
        _log(`Sending CLEAR_ALL message to ${tabs.length} tabs for cleanup.`);
        tabs.forEach(tab => {
            if (tab.id) {
                ...
                chrome.tabs.sendMessage(tab.id, { action: ACTIONS.CLEAR_ALL })
                    .catch(err => _log(`INFO: Could not send CLEAR_ALL to tab ${tab.id} (likely closed or unresponsive):`, err.message));
            }
        });
    } catch (error) {
        _log("ERROR: Failed to query tabs or send CLEAR_ALL message during cleanup:", error);
    }
    ...
    _log("Background script cleanup complete.");
}
...
chrome.runtime.onSuspend.addListener(() => {
    _log("Extension suspending (onSuspend event). Triggering cleanup.");
    cleanupExtension();
});
